...
export async function GET() {
  try {
    ...
    const detail = err instanceof Error ? err.message : "Orders error";
...
export async function PATCH(req: Request) {
  try {
    ...
    const detail = err instanceof Error ? err.message : "Order update failed";
    return NextResponse.json({ error: detail }, { status: 500 });
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    const detail = err instanceof Error ? err.message : "Order delete failed";
    return NextResponse.json({ error: detail }, { status: 500 });
  }
}
